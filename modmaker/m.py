from .cp import i2cpx_utf8, cp_get_utf8
...
def get_method(cf, cp_cache, name, desc):
	for m in cf[0x0d]:
		if not cp_get_utf8(cp_cache, int.from_bytes(m[0x01])).__eq__(name) or not cp_get_utf8(cp_cache, int.from_bytes(m[0x02])).__eq__(desc):
			continue
