...
def apply(side_name):
	...
	print(f'Patched {side_name}:{c_name}.class → net.minecraft.item.BlockItem')
...
def _modify_use_on_block_method(cf, cp_cache, side):
	...
	code = assemble_code(cf, cp_cache, side, 273, [
	...
	a_code[0x03] = a_code[0x03][0:131] + (
		(int.from_bytes(a_code[0x03][131:133], signed=True) + len(code)).to_bytes(2, signed=True)
	) + a_code[0x03][133:166] + (
		(int.from_bytes(a_code[0x03][166:168], signed=True) + len(code)).to_bytes(2, signed=True)
	) + a_code[0x03][168:273] + code + a_code[0x03][273:277]
