...
def apply(side_name):
	...
	c_name = 'com/thebluetropics/crabpack/ActionsUpdatePacket'
	...
	cf[0x0d].append(_create_constructor_b(cf, cp_cache, side, c_name))
	...
	print(f'Created {side_name}:{c_name}.class')
...
def _create_constructor_b(cf, cp_cache, side, c_name):
	m = create_method(cf, cp_cache, ['public'], '<init>', '(I)V')

	code = assemble_code(cf, cp_cache, side, 0, [
		'aload_0', ['invokespecial', ('ki', 'gt'), '<init>', '()V'],
		'aload_0', 'iload_1', ['putfield', c_name, 'actions', 'I'],
		'return'
	])
	a_code = a_code_assemble([
		(2).to_bytes(2),
...
def _create_read_method(cf, cp_cache, side, c_name):
	...
	code = assemble_code(cf, cp_cache, side, 0, [
		'aload_0',
		'aload_1', ['invokevirtual', 'java/io/DataInputStream', 'readInt', '()I'],
...
def _create_write_method(cf, cp_cache, side, c_name):
	...
	code = assemble_code(cf, cp_cache, side, 0, [
		'aload_1',
		'aload_0', ['getfield', c_name, 'actions', 'I'],
...
def _create_apply_method(cf, cp_cache, side):
	...
	code = assemble_code(cf, cp_cache, side, 0, [
		'aload_1',
		'aload_0',
		['invokevirtual', ('ti', 'me'), 'onActionsUpdate', '(Lcom/thebluetropics/crabpack/ActionsUpdatePacket;)V'],
		'return'
	])
	a_code = a_code_assemble([
		(2).to_bytes(2),
